...
if ls "$HISTORY_DIR"/*.json >/dev/null 2>&1; then
  ls -1t "$HISTORY_DIR"/*.json | head -n 50 | while read -r f; do
    entry="$(jq -c '{timestamp, phase, status, pass_rate: .summary.pass_rate, git_commit: .git.commit, critical: .summary.critical, warnings: .summary.warnings}' "$f" 2>/dev/null || echo '{}')"
...
jq -n \
  ...
  '{
    generated_at: $ts,
    repo: {owner: $owner, name: $name, branch: $branch, commit: $commit},
    totals: $totals,
    phases: $phases[0],
    history: $history[0]
  }' >"$OUT"
