...
allowed_mutations = {"image", "deploymentId", "serviceVersion", "imageDigest"}
...
if before_path and before_path.exists():
    ...
    changed_keys = sorted(
        {
            key
            for key in (set(old.keys()) | set(data.keys()))
            if old.get(key) != data.get(key)
        }
    )
    illegal = [k for k in changed_keys if k not in allowed_mutations]
    if illegal:
        print(
            "Hard rule failed: only image/deploymentId/serviceVersion/imageDigest can be changed; "
            f"illegal keys: {', '.join(illegal)}"
        )
        sys.exit(1)
