...
def check_permissions(spec_files: List[Path], permissions_registry: dict) -> bool:
        ...
        with open(spec_file, 'r', encoding='utf-8') as f:
            ...
            for perm in matches:
                if perm not in registered_permissions:
                    errors.append(f"{spec_file.name}: Permission '{perm}' not registered in permissions.yml")
    ...
    if errors:
        ...
        log_success(f"All permissions are registered ({len(registered_permissions)} total)")
        return True
...
def check_flyway_versions(spec_files: List[Path], migrations_registry: dict) -> bool:
        ...
        with open(spec_file, 'r', encoding='utf-8') as f:
            ...
            for version in matches:
                if version not in registered_versions:
                    errors.append(f"{spec_file.name}: Flyway version 'V{version}' not registered in migrations.yml")
    ...
    if errors:
        ...
        log_success(f"All Flyway versions are registered ({len(registered_versions)} total)")
        return True

# 检查可观测性 label
def check_observability_labels(spec_files: List[Path], observability_registry: dict) -> bool:
    """检查 spec 中的 Loki/Prometheus label 是否符合白名单"""
    log_info("Checking observability labels...")

    # 提取白名单和黑名单
    loki_whitelist = set(observability_registry['loki']['labels_whitelist'])
    loki_blacklist = set(observability_registry['loki']['labels_blacklist'])
    prom_whitelist = set(observability_registry['prometheus']['labels_whitelist'])
            ...
            if 'Loki' in content or 'loki' in content:
                for label in loki_blacklist:
                    if re.search(rf'\b{label}\b', content, re.IGNORECASE):
                        errors.append(f"{spec_file.name}: Loki label '{label}' is blacklisted (high cardinality)")

            # 检查 Prometheus labels
            if 'Prometheus' in content or 'prometheus' in content:
                for label in prom_blacklist:
                    if re.search(rf'\b{label}\b', content, re.IGNORECASE):
                        errors.append(f"{spec_file.name}: Prometheus label '{label}' is blacklisted (high cardinality)")
