...
def main():
    ...
    migration_dir = root_dir / 'ljwx-platform-app' / 'src' / 'main' / 'resources' / 'db' / 'migration'
    ...
    for migration in migrations:
        ...
        if phase and version != f"{phase:03d}":
            warnings.append(f"Version 'V{version}' does not match Phase {phase} (expected V{phase:03d})")
    ...
    if migration_dir.exists():
        for migration in migrations:
            ...
            if not matching_files:
                warnings.append(f"Migration file not found for V{version} ({description})")
